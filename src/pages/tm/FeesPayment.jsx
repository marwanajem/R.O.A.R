...
// BACKEND: on submit, replace console.info with POST /api/events/:id/payments (multipart/form-data)
// MAX_FILE_SIZE and ALLOWED_TYPES must also be validated server-side (multer).
...
export default function FeesPayment() {
  ...
  const competitors = getCompetitorsByEvent(id).filter((c) => c.clubCode === user?.clubCode)
  const teams = getTeamsByEvent(id).filter((t) => t.clubCode === user?.clubCode)
  ...
  const indivFees = competitors.length * event.fees.individual
  const teamFees = teams.length * event.fees.team
  ...
  const onSubmit = async (data) => {
    ...
    setFileError('')
    // In production: POST multipart form data to API
  ...
  const timelineItems = [
    { label: 'Registration open', done: true, date: event.regStart },
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div className="box">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  {[
                    { label: 'Bank', value: event.bankDetails.bank },
                    { label: 'Account Name', value: event.bankDetails.accountName },
                    { label: 'Account No.', value: event.bankDetails.accountNo },
                    { label: 'Payment Reference', value: event.bankDetails.ref.replace('{clubCode}', user?.clubCode || 'YOURCLUB') },
                  ].map(({ label, value }) => (
                    <div key={label}>
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} noValidate>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                      <Field
                        ...
                        autoComplete="off"
                        required
                        placeholder={`e.g. ${event.bankDetails.ref.replace('{clubCode}', user?.clubCode || 'YOURCLUB')}-20260604`}
