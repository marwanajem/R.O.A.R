...
// BACKEND: on submit, replace console.info with PATCH /api/events/:id/competitors/:cid
...
export default function EditCompetitor() {
  ...
  const competitor = getCompetitorsByEvent(id).find((c) => c.id === cid)
  ...
  const {
    ...
    defaultValues: competitor
      ? {
          ...
          icNumber: competitor.icNumber,
          dob: competitor.dob,
          gender: competitor.gender,
          beltGrade: competitor.beltGrade,
  ...
  if (!competitor) {
    return (
      <div className="wf">
        <TopBar breadcrumbs={[{ label: 'Events', to: '/events' }, { label: 'Competitors', to: `/events/${id}/competitors` }]} />
        <div className="wf-main"><p style={{ color: 'var(--muted)', padding: '2rem' }}>Competitor not found.</p></div>
      </div>
    )
  }

  const onSubmit = async (data) => {
    // BACKEND: PATCH /api/events/:id/competitors/:cid
    console.info('Edit competitor payload (ready for API):', { ...data, ...derived, eventId: id, id: cid })
